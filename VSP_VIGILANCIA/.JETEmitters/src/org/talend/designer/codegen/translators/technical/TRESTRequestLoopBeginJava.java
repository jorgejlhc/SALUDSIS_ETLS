...
public class TRESTRequestLoopBeginJava
{
  ...
  protected final String TEXT_10 = " =" + NL + "\t\t\t\t(java.util.Map<String, Object>) globalMap.get(\"restRequest\");" + NL + "" + NL + "\t\trestEndpoint = getRestEndpoint();" + NL + "" + NL + "\t\tif (null == requestMessage_";
  ...
  protected final String TEXT_12 = " = checkEndpointUrl(restEndpoint);" + NL + "\t\t\t\t\t// *** external thread for endpoint initialization" + NL + "\t\t\t\t\tthread4RestServiceProviderEndpoint = new Thread4RestServiceProviderEndpoint(this, endpointUrl_";
  protected final String TEXT_13 = ");" + NL + "\t\t\t\t\tthread4RestServiceProviderEndpoint.start();" + NL + "\t\t\t\t\t// *** external thread for endpoint initialization" + NL + "\t\t\t\t}" + NL + "" + NL + "\t\t\t\trestTalendJobAlreadyStarted = true;" + NL + "" + NL + "\t\t\t\tif(runInDaemonMode){" + NL + "\t\t\t\t\tThread.currentThread();" + NL + "\t\t\t\t\ttry {" + NL + "\t\t\t\t\t\twhile(true) {" + NL + "\t\t\t\t\t\t\tThread.sleep(60000);" + NL + "\t\t\t\t\t\t}" + NL + "\t\t\t\t\t} catch (InterruptedException e_";
  protected final String TEXT_14 = ") {" + NL + "\t\t\t\t\t\t// e_";
  protected final String TEXT_15 = ".printStackTrace();" + NL + "\t\t\t\t\t\t// throw new TalendException(e_";
  protected final String TEXT_16 = ", \"wholeJob\", globalMap);" + NL + "\t\t\t\t\t}" + NL + "\t\t\t\t}" + NL + "\t\t\t}" + NL + "\t\t\treturn;" + NL + "\t\t}" + NL + "" + NL + "        requestMessage_";
  protected final String TEXT_17 = ".put(\"CURRENT_MESSAGE\", org.apache.cxf.jaxrs.utils.JAXRSUtils.getCurrentMessage());" + NL + "" + NL + "        Object ctx_";
  protected final String TEXT_18 = " = requestMessage_";
  protected final String TEXT_19 = ".get(\"MESSAGE_CONTEXT\");" + NL + "        if (ctx_";
  protected final String TEXT_20 = " != null && ctx_";
  protected final String TEXT_21 = " instanceof org.apache.cxf.jaxrs.impl.tl.ThreadLocalMessageContext) {" + NL + "            requestMessage_";
  protected final String TEXT_22 = ".put(\"MESSAGE_CONTEXT\", ((org.apache.cxf.jaxrs.impl.tl.ThreadLocalMessageContext)ctx_";
  protected final String TEXT_23 = ").get());" + NL + "        }";
